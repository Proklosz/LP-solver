...
def SubProblem(data):
    sub_solver = pywraplp.Solver.CreateSolver("SCIP_MIXED_INTEGER_PROGRAMMING")
    ...
    for i in range(len(variable_values)):
        variable.append(sub_solver.IntVar(0, infinity, f"x_{i}"))
    
    

    # sub_solver.Maximize(ssMvu[0] * x1 + ssMvu[1] * x2 + ssMvu[2] * x3)
    sub_solver.Maximize(sum(ssMvu[i] * variable[i] for i in range(len(variable_values))))
    # c1 = sub_solver.Add(variable_values[0] * x1 + variable_values[1] * x2 + variable_values[2] * x3 <= RHS[0])
    c1 = sub_solver.Add(sum(variable_values[i] * variable[i] for i in range(len(variable_values))) <= RHS[0])
    ...
    pattern = SubProblemSolver(sub_solver, [variable[i] for i in range(len(variable_values))], [c1], True)
...
def MasterProblem(data, pattern_lot):
    master_solver = pywraplp.Solver.CreateSolver("SCIP_MIXED_INTEGER_PROGRAMMING")
